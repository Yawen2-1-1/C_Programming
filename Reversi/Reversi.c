...
#define PLAYONE_COLOR  WHITE //設定玩家一顏色
#define PLAYTWO_COLOR  BLACK //設定玩家二顏色
...
enum Mode {
	PLAYER, //玩家一, 玩家二 都是人 
	AI_SINGLE1, //玩家一 人, 玩家二 AI (先手) 
	AI_SINGLE2, //玩家一 AI, 玩家二 人 (後手) 
...
char key; //操作按鍵 
int speed; //移動速度 
int currentPlayer = PLAY_ONE; //設定玩家(先手) 
int currentPlayer2 = PLAY_ONE; // 設定玩家(後手) 
int modeCount = 0; //控制模式紀錄 
int totalTime = 0; //紀錄遊戲時間 
int playMode; // 設定控制模式，玩家模式或AI模式 
...
void playGame(int field[][GRID_NUM]){		
	...
	int left = 5, top = 30;
	char menu[5] = "MENU";
	char reversi[8] = "REVERSI";
	char modeChoice1[20] = "1. PLAYER vs PLAYER";
	char modeChoice2[16] = "2. PLAYER vs AI";
	char modeChoice3[16] = "3. AI vs PLAYER";
	char modeChoice4[12] = "4. AI vs AI";
	char alert[40] = "Please choose a mode to play Reversi...";
	setcolor(WHITE);
   	settextstyle(COMPLEX_FONT, HORIZ_DIR, 8);
   	outtextxy(left + 140, top, reversi);
   	outtextxy(left + 200, top + 80, menu);
   	settextstyle(COMPLEX_FONT, HORIZ_DIR, 3);
   	setcolor(RED);
	outtextxy(left + 160, top + 200, modeChoice1);	      	
	outtextxy(left + 160, top + 240, modeChoice2);
	outtextxy(left + 160, top + 280, modeChoice3);
	outtextxy(left + 160, top + 320, modeChoice4);
	settextstyle(COMPLEX_FONT, HORIZ_DIR, 3);
	setcolor(YELLOW);
	outtextxy(left + 10, top + 450, alert);
	...
	while(true){
		...
		if(playMode == PLAYER){ // playMode == PLAYER
			chessLocPtr = getChessDown(field, &focus);
		}else if(playMode == AI_SINGLE1){ // playMode == AI_SINGLE1
			if(currentPlayer == PLAY_ONE)
				chessLocPtr = getChessDown(field, &focus);
			else
				chessLocPtr = PLAYONE_AI(field, &focus, validated_locs);			
		}else if(playMode == AI_SINGLE2){ // playMode == AI_SINGLE2
			if(currentPlayer2 == PLAY_ONE)
				chessLocPtr = PLAYONE_AI(field, &focus, validated_locs);
			else
				chessLocPtr = getChessDown(field, &focus);
		...
		// AI_SINGLE2
		if(currentPlayer2 == PLAY_ONE){
			currentPlayer2 = PLAY_TWO;
		}else{
			currentPlayer2 = PLAY_ONE;
		}
...
void cleanValidatedLocs(NodePointer list){
	...
	while(list != NULL){
		drawCircle(list -> loc.row, list -> loc.col, BROWN);
		list = list -> next;
	}
	printf("\n");
...
void drawSquare(int row, int col, int color){
	...
	right = left + SquareWidth - 1;
   	...
   	fillellipse(lr_center, td_center, 20, 20);
...
Location* PLAYONE_AI(int field[][GRID_NUM],
					Location *focusPtr, NodePointer validated_locs){
	Location tmp= {0, 0};
	int maxx = 0;
	for (int i = 0; i < GRID_NUM; ++i)
		for (int j = 0; j < GRID_NUM; ++j)
		{
			...
			if (reverse(field, tmp, true) >= maxx 
				&& checkValidation(validated_locs, tmp) == true )
			{
